...
show_message() {
    message="$1"
    # The upstream my282 presenter uses MinUI's unrotated framebuffer path.
    # Keep transition messages in the log; the native NextUI list immediately
    # returns with the resulting service state and address.
    echo "$message" 1>&2
}
