...
#define SERIAL_GAMEDECK_PRIMARY		"/dev/ttyS2"
#define SERIAL_GAMEDECK_FALLBACK	"/dev/ttyS0"
...
static int miyoo_init_serial_input(void) {
	...
	const char *port = access(SERIAL_GAMEDECK_PRIMARY, F_OK) == 0
		? SERIAL_GAMEDECK_PRIMARY : SERIAL_GAMEDECK_FALLBACK;
	s_fd = uart_open(port);
	if (s_fd < 0) {
		return -1;
	}
	if (uart_init(s_fd, 9600, 0, 8, 1, 'N') < 0) {
		uart_close(s_fd);
		s_fd = -1;
		return -1;
	}
...
static int stick_thread_started = 0;
...
void Stick_init(void) {
	int initialized = miyoo_init_serial_input() == 0;
	...
	if (initialized && pthread_create(&stick_pt, NULL, &readStick, NULL) == 0)
		stick_thread_started = 1;
}
void Stick_quit(void) {
	if (s_fd >= 0) miyoo_close_serial_input();
